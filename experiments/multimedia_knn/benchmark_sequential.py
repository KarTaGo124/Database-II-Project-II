...
BENCHMARK_DB_DIR = PROJECT_ROOT / "experiments" / "multimedia_knn" / "benchmark_db_seq"
...
def create_database():
    ...
    db = DatabaseManager(
        database_name="benchmark_db_seq",
...
def create_multimedia_index(db: DatabaseManager):
    ...
    print(f"[OK] Created MULTIMEDIA_SEQ index with SIFT (took {index_time:.2f} ms)")
    return index_time
...
def save_results_to_files(results: List[Dict], output_dir: Path):
    ...
    json_file = output_dir / f"sequential_results_{timestamp}.json"
...
def main():
    print("=" * 60)
    print("MyIndex Multimedia Sequential KNN Benchmark")
    print("=" * 60)
